...
TEST(SanitizerStacktracePrinter, RenderSourceLocation) {
  ...
  str.clear();
  RenderSourceLocation(&str, "/dir/file.cc", 10, 5, "/dir/", false);
  EXPECT_STREQ("file.cc:10:5", str.data());

  str.clear();
  RenderSourceLocation(&str, "/dir/file.cc", 10, 5, "/dir/", true);
  EXPECT_STREQ("file.cc(10,5)", str.data());
}
...
TEST(SanitizerStacktracePrinter, RenderFrame) {
  ...
  // Make sure we don't print VS-style if we have no column info
